...
int16_t ADS1X15::getValue()
{
  ...
  if (_bitShift)
    raw >>= _bitShift; // Shift 12-bit results
  return raw;
}
...
void ADS1X15::_requestADC(uint16_t readmode)
{
  ...
  if (_compMode)
    config |= ADS1X15_COMP_MODE_WINDOW; // bit 4      comparator modi
  else
    config |= ADS1X15_COMP_MODE_TRADITIONAL;
  if (_compPol)
    config |= ADS1X15_COMP_POL_ACTIV_HIGH; // bit 3      ALERT active value
  else
