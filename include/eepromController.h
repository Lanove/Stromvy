...
private:
    ...
    unsigned long lastHighestHeapUsage;                  // EEPROM address 100
    array<byte, EEPROM_TOTAL_ADDRESS> outputBuffer; // used to store temporary values on readBytes and writeBytes
    array<byte,sizeof(highestHeapUsage)> buffer;
public:
    void begin();  // Init
